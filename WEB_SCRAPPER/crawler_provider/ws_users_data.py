...
def insert_update_user(id_user, name_user, place_user):

    conn = pymysql.connect(host=HOST, unix_socket=UNIX_SOCKET, user=DBUSER, passwd=DBPASSWRD, db=DB)
    cursor = conn.cursor()

    select_cmd = """SELECT * FROM users WHERE id=(%s)"""
    insert_cmd = """INSERT INTO users VALUES(%s,%s,%s)""";
    update_cmd = """UPDATE users SET name = (%s), place = (%s) WHERE id = (%s)""";

    result_sel = cursor.execute(select_cmd, (id_user))

    if(result_sel == 0):
        result_ins = cursor.execute(insert_cmd, (id_user, name_user, place_user))
        conn.commit()
    else:
        result_upd = cursor.execute(update_cmd, (name_user, place_user, id_user))
        conn.commit()
